...
@pytest.fixture
def loki_handler_mock():
    with patch("logging_loki.LokiHandler.handle") as h:
        yield h


def test_no_endpoints_on_loki_not_ready(context, loki_handler_mock):
    ...
    loki_handler_mock.assert_not_called()


def test_endpoints_on_loki_ready(context, loki_handler_mock):
    state = scenario.State(
        containers=[
            scenario.Container(
                "loki",
    ...
    with context.manager("update-status", state) as mgr:
        charm = mgr.charm
        assert charm.logging_endpoints == ["http://fqdn:80/loki/api/v1/push"]
        logging.getLogger("foo").debug("bar")

    loki_handler_mock.assert_called()

    for call in loki_handler_mock.call_args_list:
        record = call.args[0]
