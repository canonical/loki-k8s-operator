...
@pytest.mark.skip(
    "tracing-relation-joined fails - https://github.com/canonical/cos-coordinated-workers/issues/19"
)
async def test_workload_tracing_is_present(ops_test, loki_charm):
    ...
    await ops_test.model.deploy(
        "tempo-worker-k8s", application_name="tempo-worker", channel="1/edge", trust=True
    )
    await ops_test.model.deploy(
    ...
    action = await s3_integrator_leader.run_action(
        "sync-s3-credentials", **{"access-key": minio_user, "secret-key": minio_pass}
    )
    await action.wait()
    ...
    await ops_test.model.add_relation(f"{app_name}:workload-tracing", f"{TEMPO_APP_NAME}:tracing")
    # but we also need anything to come in to loki so that loki generates traces
    await ops_test.model.add_relation(f"{TEMPO_APP_NAME}:logging", f"{app_name}:logging")
    await ops_test.model.wait_for_idle(
        apps=[app_name, TEMPO_APP_NAME, "tempo-worker"], status="active", idle_period=30
    )
