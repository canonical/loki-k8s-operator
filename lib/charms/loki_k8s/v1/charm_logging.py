...
- ``juju_model``: name of the juju model this charm is deployed to
- ``juju_model_uuid``: uuid of the model
- ``juju_application``: juju application name (such as 'mycharm')
- ``juju_unit``: unit name (such as 'mycharm/0')
- ``juju_charm_name``: name of the charm (whatever is in metadata.yaml) under 'name'.
...
def _get_server_cert(
    ...
    if not sc_path.exists():
        raise RuntimeError(
            f"{charm}.{server_cert_getter} returned bad path {server_cert!r}: file not found."
        )
...
def _setup_root_logger_initializer(
    ...
    def wrap_init(self: CharmBase, framework: Framework, *args, **kwargs):
        ...
        labels = {
            **{f"juju_{key}": value for key, value in juju_topology.as_dict().items()},
...
def log_charm(
    ...
    Usage:
    >>> from charms.loki_k8s.v1.charm_logging import log_charm
...
def _autoinstrument(
    ...
    >>> from charms.loki_k8s.v1.charm_logging import _autoinstrument
