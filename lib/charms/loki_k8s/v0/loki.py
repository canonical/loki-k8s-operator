...
- Name of the relation that the Loki charm uses to interact with
  its clients. This relation must confirm to the
...
1.- Is in charge of setting the Loki Push API Address into
...
2.- Every time a Loki client unit joins a relation set its
[alerts rules](https://grafana.com/docs/loki/latest/rules/#alerting-rules) to
relation data (read more in `Alerting Rules`).
...
This Loki charm interacts with its clients using its
charm library. Charms seeking to send log to the Loki charm,
...
is automatically replaced by the `LokiConsumer` following Loki Clients's Juju
...
obtain information regarding it Loki Push API and alert rules.

Units of consumer charm sends their alert rules over app relation
